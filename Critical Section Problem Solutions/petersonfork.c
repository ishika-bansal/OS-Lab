...
int	shmID1,shmID2, 		// identifier for shared memory to store processNumber
	shrID,favID; 		// identifier for shared memory to store shared value
key_t 	key1 = 54991, key2 = 784519,key3 = 560501,key4= 565501;
int	*shared, *favoured; 
...
int main() { 
    ...
    favID = shmget(key4, sizeof(int), IPC_CREAT | 0660); // flag 
    if ( favID < 0 ) {
        printf("Shared memory 2 could not be allocated. Quitting.\n");
        exit(1); 
    } 
    ...
    // Initialization is done in parent process
        ...
        favoured = (int *)shmat(favID, NULL, 0); 
        if (favoured == (int *)-1 ) {
            printf("Failed to attach shared variable to first process. Quitting.\n");
            exit(-1); 
        } 
	...
	// This is the code first process executes
 	...
 	for(int i=0; i < 20; i++) { 
		*shared = *shared % 5;
        	*p1_wantstoenter = 1;
        	*favoured = 2;
        	while(*p2_wantstoenter && *favoured==2)
        		;
	...
	// This is the code second process executes
 	...
 	for(int i=0; i < 20; i++) { 
        	...
        	*favoured=1;
        	...
        	while(*p1_wantstoenter && *favoured==1)
        		;
