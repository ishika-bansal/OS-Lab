...
void BEST_FIT(info a[],int n, int m){
	vector <int> fail;
	vector <int> success;
	int f = 0;
	...
	while(1){
		...
		if(a[i].time_entry==timer){
			...
			if(!a[i].first_exec){
				//cout<<memory_available<<" AVAILABLE MEMORY!!!"<<endl;				
				cout<<i+1<<" PROCESS COULDN'T BE GIVEN THE SPACE. MEMORY ALLOTMENT FAIL."<<endl;
				faill++;
	   ...
	   // timer++;
		...
		float e = memory_available;
		cout<<"EXTERNAL FRAGMENTATION (% OF FREE SPACE) : "<<(e/m)*100<<"%"<<endl;
...
//Reading the page reference string
    ...
    BEST_FIT(table,proc_num,memory);
}
